...
# Section 1: Recreating The Economist's chart on annual change in population in The pecking order of the world’s population is soon to change (14 July 2022), https://blog.datawrapper.de/wp-content/uploads/2022/07/image6-2.png
...
# Section 2: Ireland's population fertility compared to EU27 countries, using John Burn-Murdoch's Financial Times graphic style, https://twitter.com/jburnmurdoch/status/1539248155446980608
...
p3 = df3_long %>% 
  ggplot() +
  geom_vline(xintercept=2022, size=.2, color="grey70") +
geom_line(aes(x=Time, y=value, linetype=name), size=.4) +
  ggbraid::geom_braid(aes(x=Time, ymin=`Crude Birth Rate`, ymax=`Crude Death Rate`, fill=`Crude Birth Rate`<`Crude Death Rate`), data=df3, alpha = 0.85, method = 'line')+
  facet_geo(~Location,grid = "eu_grid1") +
  ...
  theme(text=element_text(family=f1),
        ...
        plot.subtitle=element_text(family=f2, size=9, margin=margin(b=7)),
        plot.title=element_text(family=f3, size=13),
        plot.title.position = "plot",
        plot.caption.position = "plot",
        plot.caption = element_text(family=f2,hjust=0),
        plot.background=element_rect(fill="#f8f7ff", color=NA),
        plot.margin=margin(.4,.4,.4,.4,unit="cm")
        ) +
  guides(linetype=guide_legend(order=1)) +
  labs(caption="Source: population.un.org",
       title="Births and Deaths, 1950 - 2100",
       subtitle="EU27 + UK, Crude Births Rate (births per 1,000 population) and Crude Death rate (deaths per 1,000 population) according to\nUN World Population Prospects 2022, medium projection scenario")
